...
All four compartments are carried in a single mixed space $P = V_S \times V_I \times V_R \times V_W$ built from four degree-1 Lagrange elements. The bilinear form
\begin{equation}
a(U,\Psi) = \sum_{X\in\{S,I,R\}} \int_{\Gamma_h}\Big[\,u_X \psi_X + \Delta t\,(D_X\,\Grad u_X)\cdot\Grad \psi_X\Big]\,dA \;+\; \int_{\Gamma_h} u_W \psi_W\, dA
\label{eq:bilinear}
\end{equation}
depends only on the mesh geometry, the land cover and the diffusion parameters --- not on the current solution --- so it is assembled and factorized \emph{once}, before the time loop, and reused unchanged at every step. Mass terms are integrated with the vertex rule of Section~\ref{sec:fem}, giving $M_L$; stiffness terms use a Gauss rule. We write $B$ for the resulting operator.
