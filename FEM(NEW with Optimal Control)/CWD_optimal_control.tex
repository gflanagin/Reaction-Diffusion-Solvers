...
\begin{equation}
  ...
  \label{eq:K}
\end{equation}
where $c(\mathbf{x})$ takes one of a discrete set of values determined by the land cover classification at $\mathbf{x}$, as listed in Table~\ref{tab:landcover}. Forested and mixed edge habitat is expected to support substantially higher deer densities than open agricultural or developed land, and the coefficients are calibrated against regional deer density estimates so that the population-weighted mean of $K$ reproduces observed densities across the study region. Since $c$ is piecewise constant on the land cover classes, $K$ is discontinuous across class boundaries; this poses no difficulty for the weak formulation of Section~\ref{sec:fem}, in which $K$ enters only through a zeroth-order term.

\begin{table}[h]
\centering
\caption{Land cover scaling coefficients defining $K(\mathbf{x}) = K_0\,c(\mathbf{x})$. Classes follow the MRLC National Land Cover Database classification. The adopted values, together with the companion permeability column used in the diffusion tensor, are given in Table~\ref{tab:landcover-values}.}
\label{tab:landcover}
\begin{tabular}{@{}ll@{}}
\toprule
Land cover class & Scaling coefficient $c$ \\
\midrule
[class 1] & [value] \\
[class 2] & [value] \\
[class 3] & [value] \\
...
\label{eq:ocp}
\end{equation}
Note that $\hat J$ is not convex in $v$: the state system is nonlinear (through $SR$, $SW$, $SN/K$, and $vR$), so $v\mapsto y(v)$ is nonlinear and \eqref{eq:ocp} may in principle possess several local minimizers. The method of Section~\ref{sec:algorithm} is a descent method and therefore locates a stationary point; a good practice is to re-run from several initial guesses.
...
Neither method escapes the caveat noted after \eqref{eq:ocp}: $\hat J$ is not convex, so both locate a stationary point rather than a global minimizer, and re-running from several initial guesses remains good practice.
