...
\subsection{Terrain/Land-cover aware Diffusion Tensor \texorpdfstring{$D$}{D}}
\label{sec:diffusion}

Deer movement, and hence the effective diffusivity of $S$, $I$, and $R$, depends on both land cover and local terrain steepness. (As noted above, $W$ does not diffuse.) To model realistic movement of deer, we handle diffusion in the slope direction separately from the contour direction. In this way, we can limit the diffusion along the slope direction depending on terrain steepness while allowing free diffusion in the contour direction. Additionally, we include a permeability factor to limit diffusion depending on local land cover to model movement through various environments. The terrain and land-cover aware diffusion tensor is constructed as follows:

\begin{equation}
  D(\mathbf{x}) = \ell(\mathbf{x})(D_c(\mathbf{x})\mathbf{e_{c}e_{c}^T} + D_s(\mathbf{x})\mathbf{e_{s}e_{s}^T})
  \label{eq:D}
\end{equation}

Where $\mathbf{e_s}$ is the normalized projection of the direction of gravity onto the surface at $\mathbf{x}$, $\mathbf{e_c}$ is a normalized surface direction perpendicular to $\mathbf{e_s}$, and $\ell$ is the permeability of movement through the local land cover class. Below the local slope and contour dependent factors $D_s(\mathbf{x})$ and $D_c(\mathbf{x})$ are described below.
...
\paragraph{Contour dependence.}
For the present study the contour component is taken to be spatially constant,
\begin{equation}
...
\caption{Slope-dependent diffusivity $D_s$ as a function of terrain slope angle $\phi$, normalized with $\kappa_2=1$}
\label{fig:Ds}
\end{figure}
...
$\kappa_2$ & Baseline (land-cover) diffusivity             & m$^{2}$\,yr$^{-1}$              & Table~\ref{tab:paramvalues} \\
$\kappa_1$ & Slope diffusivity scale, $\kappa_2(1+e^{-2})$ & m$^{2}$\,yr$^{-1}$              & derived \\
\bottomrule
\end{tabular}
\end{table}
